...
contract('Flight Surety Tests', async (accounts) => {
  ...
  it('airline is only operational when it has submitted funding of 10 ether', async () => {
    ...
    try{
        await config.flightSuretyData.fund({from: admin2, value: fundPrice});
        await config.flightSuretyData.fund({from: admin3, value: fundPrice});
    ...
    try{
        ...
        if(registration_status[0] == false && registration_status[1] == false){
            await config.flightSuretyData.approveAirlineRegistration(admin4, true, {from: config.owner});  //vote out
            await config.flightSuretyData.approveAirlineRegistration(admin4, true, {from: admin3});         // vote in
            await config.flightSuretyData.approveAirlineRegistration(admin4, false, {from: admin2});         // vote in
        }
  ...
  it('Insured passenger can be credited if flight is delayed', async () => {
    ...
    try{
        ...
        credit_before = await config.flightSuretyData.getUserCredit.call({from: passenger});
        ...
        credit_after = await config.flightSuretyData.getUserCredit.call({from: passenger});
    ...
    try{
        ...
        balance_before = await config.flightSuretyData.getUserCredit.call({from:passenger})
        ...
        balance_after = await config.flightSuretyData.getUserCredit.call({from:passenger})
