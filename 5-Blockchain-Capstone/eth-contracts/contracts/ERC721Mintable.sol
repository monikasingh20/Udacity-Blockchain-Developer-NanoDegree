...
contract Ownable {
    ...
    modifier onlyOwner() {
        require(msg.sender == _owner, "Access not allowed to contract owner");
        _;
    }
    event Ownershiptransfer(address indexed old_owner, address indexed new_owner);

    function transferOwnership(address new_owner) public onlyOwner {
        // TODO add functionality to transfer control of the contract to a newOwner.
        // make sure the new owner is a real address
        require(new_owner != address(0), "New owner address is not a real address");
        emit Ownershiptransfer(_owner, new_owner);
        _owner = new_owner;
    }
}
...
contract Pausable is Ownable {
    ...
    event Paused(address indexed actionBy);
    event Unpaused(address indexed actionBy);
...
//    @dev Approves another address to transfer the given token ID
    function approve(address to, uint256 tokenId) public {
        ...
        require(to != _tokenOwner[tokenId],"Address is owner of token");
        // TODO require the msg sender to be the owner of the contract or isApprovedForAll() to be true
        require((msg.sender == _tokenOwner[tokenId]) || (isApprovedForAll(_tokenOwner[tokenId], msg.sender)),"Address to be the owner of the contract or ApprovedForAll");
    ...
    function _transferFrom(address from, address to, uint256 tokenId) internal {
        ...
        require(from == _tokenOwner[tokenId], "from address is the owner of the given token");
...
contract CustomERC721Token is ERC721Metadata("Custom Contract","CC","https://s3-us-west-2.amazonaws.com/udacity-blockchain/capstone/") {

    function mint(address to, uint256 tokenId) public whenNotPaused onlyOwner returns(bool)
    {
        super._mint(to, tokenId);
